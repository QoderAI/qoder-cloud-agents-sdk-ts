...
```ts
import ForwardClient, { PATCredential } from 'qca-sdk/forward';
...
CommonJS 可使用 `const { ForwardClient } = require('qca-sdk')`。客户端与所有业务类型也可从 `qca-sdk/forward` 导入；包根导出客户端和通用类型，业务类型使用模式入口。
...
```ts
import { APIError, APIUserAbortError } from 'qca-sdk/forward';
...
```ts
import { toFile } from 'qca-sdk/forward';
...
类型名从 `qca-sdk/forward` 导入；签名中的 `APIPromise`、`PagePromise`、`Stream` 为 SDK 通用类型。类型链接可以查看响应的全部字段、枚举、联合类型和请求中的嵌套结构。位置参数与 `params` 中标为 path 的字段会做 URL 编码；header 字段不会进入 JSON body。
