...
for (const c of [
  ...
  { name: 'thinking block is not the answer', events: [{ type: 'agent.thinking', content: [{ type: 'text', text: 'marker' }] }, { type: 'session.status_idle', stop_reason: { type: 'end_turn' } }], valid: false },
...
for (const raw of ['{"type":"session.error"}', '{"type":"session.status_terminated"}', '{"type":"session.status_idle","stop_reason":{"type":"max_tokens"}}', '{"type":"session.status_idle","stop_reason":"max_tokens"}', '{broken']) test(`execution rejects failure ${raw}`, () => assert.throws(() => new TurnResult().observe(raw)));
