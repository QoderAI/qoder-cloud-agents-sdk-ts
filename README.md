...
Each entry point can also be imported on its own — `qoder-cloud-agents-sdk-ts/forward` and `qoder-cloud-agents-sdk-ts/managed` — both providing a default client and named exports. The `*Api` and `*Raw` methods of the previous OpenAPI Generator output are gone, as are APIs outside the Go SDK surface such as Service Account Token, Managed Search and Webhook.
